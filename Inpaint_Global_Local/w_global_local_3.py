...
#D.compile(loss= wasserstein, optimizer=Adam())#, metrics=['accuracy'])

G = build_generator(global_shape)



real_input = Input(global_shape)
fake_input = Input(global_shape)

real_field = inpaint_crop(real_input, coord, pad_size)


fake_img = G(fake_input)
fake_field = inpaint_crop(fake_input, coord, pad_size)

interpolated_img = RandomWeightedAverage(batch_size)(inputs = [real_input, fake_img])
interpolated_field = inpaint_crop(fake_input, coord, pad_size)

real_validity = D([real_field, real_input])
fake_validity = D([fake_field, fake_img])
interpolated_validity = D([interpolated_field, interpolated_img])
gp = GradientPenalty()([interpolated_validity,interpolated_field])


critic = tf.keras.Model(inputs = [real_input,fake_input], outputs = [real_validity,fake_validity,gp])

D.trainable = True
G.trainable = False 
critic.compile(loss = [wasserstein,wasserstein,'mse'],loss_weights = [1,1,10],optimizer =Adam(lr=0.0001,beta_1=0.5,beta_2=0.9))#,metrics=['accuracy'])
...
GL.compile(loss=['mse', wasserstein], loss_weights=[0.9996, 0.0004], optimizer=Adam(lr=0.0001,beta_1=0.5,beta_2=0.9)) #,metrics=['accuracy'])
...
for epoch in range(epochs):
    ...
    for train_batch in iter(train_ds):
        ...
        #D.trainable = True
        for _ in range(1):
            d_loss = critic.train_on_batch([train_batch, masked_imgs],[real,fake,dummy])
        print("D:",d_loss)
