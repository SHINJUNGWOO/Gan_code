...
class GAN():
    ...
    def conv_bn(self,img, kernel_size,dilated, strides, filters, padding="same"):
        x = img
        if strides <0:
            x = UpSampling2D()(x)
            strides = 1
    ...
    def random_crop_layer(self,real_inputs):
        ...
        tmp_np = np.ones((self.batch_size,self.input_dim[0],self.input_dim[1],3),dtype="float32")
        for i in range(self.batch_size):
            ...
            tmp_np[i,a_1:a_2,b_1:b_2,:]=[0.,0.,0.]
        ...
        #masked_img = tf.image.draw_bounding_boxes(real_inputs, boxes,  [0.0, 0.0, 0.0])
        masked_img = Lambda(lambda x :tf.math.multiply(x[0],x[1]))((real_inputs,tmp_np))
        
        return real_inputs, masked_img, mask
    @tf.function
    ...
    def stage_1_model(self):
        ...
        x = self.conv_bn(x, 3, 1, 1, kernel_norm*2)
        #x = MaxPooling2D()(x)
        x = self.conv_bn(x, 5, 1, 1, kernel_norm*2)
        ...
        x = self.conv_bn(x, 3, 1, 1, kernel_norm*4)
        x = self.conv_bn(x, 3, 1, -2, kernel_norm*2)
        x = self.conv_bn(x, 3, 1, 1, kernel_norm*1)
        #x = MaxPooling2D()(x)
        x = self.conv_bn(x, 3, 1, 1, 24)
        x = self.conv_bn(x, 3, 1, 1, 3)
    ...
    def stage_2_model(self):
        ...
        x = self.conv_bn(x, 3, 1, 1, kernel_norm*2)
        x = self.conv_bn(x, 3, 1, -2, kernel_norm*2)
        x = self.conv_bn(x, 3, 1, 1, kernel_norm)
        #x = MaxPooling2D()(x)
        x = self.conv_bn(x, 3, 1, 1, 24)
        x = self.conv_bn(x, 3, 1, 1, 3)

        self.stage_2 = Model(input_img, x)
    # Stage 1

    def generator_model(self):
        input_img = Input(shape=self.input_dim)

        stage_out = self.stage_1(input_img)
        output_img = self.stage_2(stage_out)

        self.generator = Model(input_img, output_img)

    def local_discriminator_model(self):
        input_img = Input(shape=self.blank_dim)
        x = input_img
        x = self.conv_bn(x,5,1,2,64)
        x = self.conv_bn(x,5,1,2,256)
        x = self.conv_bn(x,5,1,2,256)
        x = self.conv_bn(x,5,1,2,64)
        x = Flatten()(x)
    ...
    def critic_model(self):
        input_img = Input(shape=self.input_dim)
        real, crop_img, mask = Lambda(self.random_crop_layer)(input_img)
        fake = self.generator(crop_img)
        ...
        self.gp = GradientPenalty()([valid_inter,interpolate])

        self.critic = Model(input_img,[valid_real,valid_fake,valid_inter])
    ...
    def stage_1_compile(self):
        self.stage_2.trainable =False
        self.discriminator.trainable = False
        self.stage_1.trainable = True

        self.stage_1.compile(
    ...
    def train(self,epoch,train_data):
        ...
        for epoch_num in range(1,epoch+1):
            ...
            s_1_loss = self.stage_1.train_on_batch(train_img,train_img)
    ...
    def save_img(self,img,epoch):
        real,fake,mask = self.random_crop_layer(img)
        save_img(self.img_save_dir+"blanked_img"+str(epoch)+".jpg",fake)
        save_img(self.img_save_dir+"reconst"+str(epoch)+".jpg",self.generator.predict(fake))
