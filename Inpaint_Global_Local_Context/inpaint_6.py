...
class inpainting():
    def __init__(self,
        ...
        self.mask_size = local_shape[0]* local_shape[1]
    ...
    def block_patch(self,input_, mode = 'random_box', margin=10):
        ...
        if mode == 'central_box':
            ...
            h_ = tf.constant([self.global_shape[0]//4], dtype=tf.int32)[0]
            w_ = tf.constant([self.global_shape[1]//4], dtype=tf.int32)[0]
    ...
    #     back_ground_patch = tf.image.extract_patches(img, sizes=(1, 1, 1, 1),
    #                                                  strides=(1, self.extract_rate, self.extract_rate, 1),
    #                                                  rates=(1, 1, 1, 1), padding="SAME")
    #     inverted_mask = tf.image.extract_patches(inverted_mask, sizes=(1, 1, 1, 1),
    #                                              strides=(1, self.extract_rate, self.extract_rate, 1),
    #                                              rates=(1, 1, 1, 1), padding="SAME")
    ...
    def refine_model(self):
        ...
        boolen_mask = tf.cast(input_mask,dtype=tf.float32)
        x_mask = tf.concat([boolen_mask,boolen_mask,boolen_mask],axis=-1)
        x,x_mask = PConv2D(32,5,strides=1,padding="same",activation='relu')([input_,x_mask])
        x, x_mask = PConv2D(64, 5, strides=2, padding="same", activation='relu')([x, x_mask])
        x, x_mask = PConv2D(64, 5, strides=1, padding="same", activation='relu')([x, x_mask])
        x, x_mask = PConv2D(128, 5, strides=2, padding="same", activation='relu')([x, x_mask])
        x, x_mask = PConv2D(128, 5, strides=1, padding="same", activation='relu')([x, x_mask])
        x, x_mask = PConv2D(128, 5, strides=1, padding="same", activation='relu')([x, x_mask])



        #context = self.context_attention(x,mask)
        #context = Lambda(lambda inp:self.context_attention(inp[0],inp[1]))((x,x_mask))
        context = self.context_attention(x,x_mask)
...
test = inpainting(
    ...
    local_shape=(64, 64, 3),
    weight_save_dir="./data/weight/inpaint_attention_5/",
    img_save_dir = "./data/inpaint_attention_5/")
